...
const Library = () => {
    ...
    const fetchLibraryData = async () => {
      try {
        ...
        const response = await fetch(
          `${API_CONFIG.BASE_URL}/research/get-published-research-library`,
          {
            method: "GET",
            headers: {
  ...
  const handleViewDetails = async (researchId) => {
    ...
    try {
      const token =
        localStorage.getItem("token") || localStorage.getItem("authToken");
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/research/get-published-research-details-library/${researchId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <DashboardLayout>
      <div className="p-8 lg:p-12 max-w-[1600px] mx-auto w-full text-slate-800 dark:text-slate-100 bg-white dark:bg-[#0b0f0c]">
        {/* Header Section */}
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-slate-900 dark:text-slate-900 dark:text-white">
            {" "}
            Research Library & Discovery
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-2 text-lg">
            {" "}
            Explore thousands of publications and studies from our global
            network.
          </p>
        </div>
...
focus:ring-4 focus:ring-[#10b981]/10 
rounded-2xl pl-14 pr-6 py-4 text-lg transition-all shadow-sm outline-none 
...
placeholder-slate-400 dark:placeholder-slate-500"
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white dark:bg-[#181818] rounded-2xl border border-slate-200 dark:border-white/5">
            {" "}
            <MaterialIcon
              name="search_off"
              className="text-6xl text-slate-600 mb-4 block"
            />
...
rounded-2xl p-6 shadow-sm flex flex-col h-full group 
transition-all duration-300 
hover:-translate-y-1.5 
hover:border-[#10b981]/30 
hover:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1),0_0_15px_-3px_rgba(16,185,129,0.15)]"
              >
                <div className="flex justify-between items-start mb-6">
                  <span className="text-[11px] font-bold text-[#10b981] uppercase tracking-wider bg-[#10b981]/10 px-2 py-1 rounded-md">
                    Published
                  </span>
                  <button className="text-slate-500 hover:text-[#10b981] transition-colors"></button>
                </div>

                <h3 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-[#10b981] transition-colors line-clamp-2 mb-6 capitalize">
                  {pub.research_title}
                </h3>
                ...
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <img
                    ...
                    alt={pub.name}
                    className="w-10 h-10 rounded-full border border-slate-200 dark:border-white/10 shrink-0 object-cover"
                    onError={(e) => {
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#10b981]/80 font-medium capitalize">
                      {pub.user_type === "institute"
                        ? pub.institute_name || "Institute Researcher"
      ...
      hover:border-[#10b981] dark:hover:border-[#10b981]
      hover:bg-[#10b981]/10 dark:hover:bg-[#10b981]/5
      hover:text-[#10b981] dark:hover:text-[#10b981]
      hover:shadow-[0_0_10px_rgba(16,185,129,0.12)]
      dark:hover:shadow-[0_0_10px_rgba(16,185,129,0.2)]
    "
                  >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 sm:p-6 overflow-y-auto">
            <div className="bg-white dark:bg-[#121212] border border-white/10 rounded-3xl w-full max-w-4xl shadow-2xl relative my-auto">
              {/* Close Button */}
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 dark:text-white bg-white/5 hover:bg-red-500/20 rounded-full w-10 h-10 flex items-center justify-center transition-colors z-10"
              >
                ...
                <div className="p-8 sm:p-10 max-h-[85vh] overflow-y-auto custom-scrollbar">
                  ...
                  <div className="mb-8 border-b border-white/5 pb-8">
                    <div className="flex gap-3 mb-4">
                      ...
                      <span
                        className="w-full bg-slate-100 dark:bg-white/5 border border-slate-300 dark:border-transparent 
text-slate-700 dark:text-slate-300 
font-bold py-3 rounded-xl text-sm 
flex items-center justify-center gap-2 
transition-all duration-200 
hover:border-[#10b981] 
hover:bg-[#10b981]/5 
hover:text-[#10b981] border border-white/10 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider"
                      >
                        {selectedDetails.research_language}
                      </span>
                    </div>
                    <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white leading-tight capitalize mb-4">
                      {selectedDetails.research_title}
                    </h2>
                    <p className="text-slate-400 text-sm">
                      Published Year:{" "}
                      <span className="text-slate-900 dark:text-white font-semibold">
                        {selectedDetails.research_year}
                      </span>{" "}
                      • ID: {selectedDetails.registration_id}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/5 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#10b981]">
                        {selectedDetails.user_type === "institute"
                          ? selectedDetails.institute_name || "Institute"
                          : "Individual Researcher"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mb-8">
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                      <MaterialIcon name="article" className="text-[#10b981]" />{" "}
                      Abstract
                    </h3>
                    <p
                      className="text-slate-300 leading-relaxed bg-slate-50 dark:bg-[#0a0a0a]
border border-slate-200 dark:border-[#1a1a1a] p-6 rounded-2xl border border-[#1a1a1a]"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedDetails.method_type?.map((method, idx) => (
                            <span
                              key={idx}
                              className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-800 dark:text-slate-200"
                            >
                              {method}
                            </span>
                          ))}
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-900 dark:text-white bg-white/5 p-3 rounded-lg border border-white/5">
                          <span className="text-slate-400">Type:</span>{" "}
                          {selectedDetails.data_type} <br />
                          <span className="text-slate-400 mt-1 block">
                            Sample Size:
                          </span>{" "}
                          {selectedDetails.simple_size}
                        </p>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="bg-[#10b981]/5 border border-[#10b981]/20 p-4 rounded-xl">
                          <p className="text-sm text-slate-900 dark:text-white capitalize">
                            <span className="font-bold text-[#10b981] uppercase text-xs">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedDetails.sdg?.map((goal, idx) => (
                            <div
                              key={idx}
                              className="w-10 h-10 bg-[#4c9f38] text-slate-900 dark:text-white font-bold flex items-center justify-center rounded-lg shadow-lg"
                            >
                              {goal}
                            </div>
                          ))}
                  ...
                  <div className="mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedDetails.keywords?.map((kw, idx) => (
                        <span
                          key={idx}
                          className="text-xs bg-[#10b981]/10 text-[#10b981] border border-[#10b981]/20 px-3 py-1.5 rounded-md"
                        >
                          #{kw}
                        </span>
                      ))}
                  ...
                  {/* Action Footer */}
                  <div className="pt-8 border-t border-slate-200 dark:border-white/5 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    ...
                    {selectedDetails.research_file && (
                      <a
                        href={`${API_CONFIG.BASE_URL}/${selectedDetails.research_file}`}
                        target="_blank"
