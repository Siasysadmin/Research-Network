...
const ReviewStatusModal = ({ researchId, onClose, remainingUpdates }) => {
  return (
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              Your research profile is currently under review by our board
              members.
            </p>
            ...
            <div className="bg-[#f8fafc] border border-[#32ff99]/20 rounded-xl p-4 space-y-3 shadow-sm dark:bg-white/5 dark:shadow-none">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                    Your research will be made public only after 3 or more board
                    members approve it
                  </p>
                </div>
              </div>
            ...
            <div className="bg-[#f8fafc] border border-gray-200 rounded-xl p-4 space-y-3 shadow-sm dark:bg-white/5 dark:border-white/10 dark:shadow-none">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                    {Number(remainingUpdates) > 0
                      ? `You will have ${remainingUpdates} chance${
                          Number(remainingUpdates) > 1 ? "s" : ""
                        } to reupload and resubmit the same research for approval.`
                      : "You have no chances left to reupload or resubmit this research."}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-[#32ff99]/10 border border-[#32ff99]/20 rounded-xl p-3">
            <p className="text-xs text-slate-700 dark:text-slate-300 text-center">
              You will be notified once the review is completez
            </p>
          </div>
        </div>
...
const ResearchUploadForm = () => {
  ...
  const isEditMode = Boolean(researchId);
  const [updateCount, setUpdateCount] = useState(0);
  const [remainingUpdates, setRemainingUpdates] = useState(0);
    ...
    const fetchResearchById = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_CONFIG.BASE_URL}/user/get-research-id/${researchId}`,
          {
            method: "GET",
            headers: {
        ...
        if (result.status && result.data) {
          ...
          setUpdateCount(Number(data.update_count || 0));
          setFormData((prev) => ({
            ...prev,
            ...
            geography: data.level || "",
            district: data.district || "",
            state: data.state || data.place || "",
            country: data.country || "",

            primaryAuthor: data.primary_author_id || data.registration_id || "",
            coAuthors: data.co_author_id
              ? Array.isArray(data.co_author_id)
                ? data.co_author_id
                : [data.co_author_id]
              : [],

            sdgGoals: Array.isArray(data.sdg) ? data.sdg.map(String) : [],
            impactTypes: Array.isArray(data.type_of_impact)
              ? data.type_of_impact
              : [],
            methodology: Array.isArray(data.method_type)
              ? data.method_type
              : [],

            simpleSize: data.simple_size || "",
            dataType: data.data_type || "",
            declaration: data.declaration_of_study || "",

            file: null,
            oldFile: data.research_file || "",
          }));

          setWordCount(
            data.abstract
              ? data.abstract.split(/\s+/).filter((word) => word.length > 0)
                  .length
              : 0,
          );
  ...
  const validate = () => {
    ...
    if (!formData.language) {
      ...
      (!formData.languageInput || !formData.languageInput.trim())
    ) {
    ...
    if (!formData.file && !formData.oldFile)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/user/research-update/${researchId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (result.status) {
        setRemainingUpdates(Number(result.data?.remaining_updates ?? 0));
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={
                        isIndividualUser ? undefined : handleInputChange
                      }
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    >
                      ...
                      {formData.file ? (
                        <div className="text-sm text-[#32ff99] font-semibold relative z-20">
                          New PDF Selected: {formData.file.name}
                        </div>
                      ) : formData.oldFile ? (
                        <div className="text-sm text-[#32ff99] flex flex-col items-center gap-1 relative z-20">
                          <span>Current PDF already uploaded</span>

                          <a
                            href={`${API_CONFIG.BASE_URL}/${formData.oldFile}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={(e) => e.stopPropagation()}
                            className="underline font-semibold"
                          >
                            View Current Research PDF
                          </a>
                        </div>
                      ) : null}
