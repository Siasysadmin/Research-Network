...
import ShareModal, { fetchShareData, sendPost, handleNativeShare } from "./ShareModal";
...
const getLocalCurrentUser = () => {
  try {
    ...
    const profileImg =
      obj.profile_image || obj.avatar || obj.profile_pic ||
      obj.profile_individual_details?.profile_image ||
      obj.profile_institute_details?.profile_image || null;
    const avatarUrl = profileImg
      ? String(profileImg).startsWith("http") ? profileImg : `${API_CONFIG.BASE_URL}/${profileImg}`
      : null;
    return { id: String(obj.id || obj.user_id || ""), name: obj.name || "", avatarUrl };
  } catch { return { id: null, name: "", avatarUrl: null }; }
};

const formatTimeAgo = (timestamp) => {
  if (!timestamp) return "Just now";
  let normalized = String(timestamp).replace(" ", "T");
  if (!normalized.endsWith("Z") && !normalized.includes("+")) normalized += "+05:30";
  ...
  <div className="flex gap-3">
    ...
    <div className="flex-1 min-w-0 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#141414] p-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h5 className="font-bold text-slate-900 dark:text-white">{exp.company}</h5>
        {hasVal(exp.duration) && (
 ...
 */
const SectionCard = memo(({ icon, title, isOpen, onToggle, hasContent, children }) => {
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="text-sm font-bold text-slate-900 dark:text-white truncate capitalize">
          {item.user_type === "institute" && item.institute_name ? item.institute_name : item.name}
        </p>
...
export const Engagement = memo(({ likeCount, commentCount }) => (
  <div className="mt-4 flex items-center gap-6 pt-4 border-t border-gray-200 dark:border-white/5">
    <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-sm font-bold">
      <MaterialIcon name="favorite" className="text-lg text-rose-500" style={{ fontVariationSettings: "'FILL' 1" }} />
      {Number(likeCount) || 0}
    </span>
    <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-sm font-bold">
      <MaterialIcon name="chat_bubble" className="text-lg text-[#00c46a] dark:text-[#00ff85]" />
...
export const PostCard = memo(({ post, onShareClick }) => {
  ...
  const [isLiked, setIsLiked] = useState(() => {
    const saved = (() => { try { return JSON.parse(localStorage.getItem("postLikes") || "{}"); } catch { return {}; } })();
    return saved[post.id] !== undefined ? saved[post.id] : post.is_liked === "1";
  });
  const [likeCount, setLikeCount] = useState(parseInt(post.like_count || 0));
  const [isSaved, setIsSaved] = useState(() => {
    const saved = (() => { try { return JSON.parse(localStorage.getItem("savedPosts") || "{}"); } catch { return {}; } })();
    return saved[post.id] !== undefined ? saved[post.id] : post.is_saved === "1";
  });
  ...
  const handleToggleLike = async () => {
    ...
    const stored = (() => { try { return JSON.parse(localStorage.getItem("postLikes") || "{}"); } catch { return {}; } })();
    stored[post.id] = !wasLiked;
    localStorage.setItem("postLikes", JSON.stringify(stored));
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/post/like-post/${post.id}`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
      const result = await res.json();
      if (!result.status) {
        setIsLiked(wasLiked); setLikeCount((c) => wasLiked ? c + 1 : Math.max(0, c - 1));
        stored[post.id] = wasLiked; localStorage.setItem("postLikes", JSON.stringify(stored));
      }
    } catch {}
  };
  ...
  const handleToggleSave = async () => {
    ...
    setIsSaved(!wasSaved);
    const stored = (() => { try { return JSON.parse(localStorage.getItem("savedPosts") || "{}"); } catch { return {}; } })();
    stored[post.id] = !wasSaved;
    localStorage.setItem("savedPosts", JSON.stringify(stored));
    if (wasSaved) window.dispatchEvent(new CustomEvent("postUnsaved", { detail: { postId: post.id } }));
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/post/save-post/${post.id}`, {
        method: "GET", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
      const result = await res.json();
      if (!result.status) {
        setIsSaved(wasSaved); stored[post.id] = wasSaved; localStorage.setItem("savedPosts", JSON.stringify(stored));
      }
    } catch {}
  };
  ...
  const fetchComments = async () => {
    const token = getLocalAuthToken();
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/post/get-comments/${post.id}`, { headers: { Authorization: `Bearer ${token}` } });
      const result = await res.json();
      const fetched = (result.data || []).map((c) => ({
        id: c.id, text: c.comment, author: c.name, authorId: c.user_id,
        authorAvatar: c.profile_image ? `${API_CONFIG.BASE_URL}/${c.profile_image}` : null, timestamp: c.created_at,
      }));
      setCommentsList(fetched); setCommentsLoaded(true);
    } catch {}
  };
  ...
  const handleAddComment = async () => {
    ...
    const temp = { id: tempId, text: clean, author: currentUser.name, authorId: currentUser.id, authorAvatar: currentUser.avatarUrl, timestamp: new Date().toISOString() };
    setCommentsList((prev) => [temp, ...prev]); setNewCommentText("");
    const token = getLocalAuthToken();
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/post/add-comment/${post.id}`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    ...
    } catch { setCommentsList((prev) => prev.filter((c) => c.id !== tempId)); }
  };

  const handleDeleteComment = async (commentId) => {
    setCommentsList((prev) => prev.filter((c) => c.id != commentId));
    const token = getLocalAuthToken();
    try { await fetch(`${API_CONFIG.BASE_URL}/post/delete-comment/${commentId}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); } catch {}
  };
    ...
    <article className="bg-white dark:bg-[#020f0a] rounded-lg sm:rounded-xl border border-white/5 shadow-sm overflow-visible relative w-full">
      <div className="p-3 sm:p-4 md:p-5">
        <div className="flex items-start gap-2 sm:gap-3 mb-3">
          <img alt={post.name || "User"} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-[#00ff88]/20 object-cover shrink-0" src={getPostProfileSrc(post)} onError={(e) => { e.target.src = avatar; }} />
          <div className="flex-1 min-w-0">
            <p className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-white truncate capitalize">{post.name || "User"}</p>
            <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 capitalize">{post.user_type === "institute" ? "Institute" : "Individual"} • {formatActivityDate(post.created_at)}</p>
          </div>
        </div>
        ...
        {tags.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {tags.map((t, i) => (<span key={i} className="text-[10px] sm:text-xs font-semibold text-emerald-600 dark:text-[#00ff88] bg-[#00ff88]/10 border border-[#00ff88]/20 px-2 py-0.5 rounded-full">#{String(t).replace(/^#/, "")}</span>))}
          </div>
        )}
        {img && (
          <div className="mb-3 rounded-lg sm:rounded-xl overflow-hidden bg-gray-100 dark:bg-black">
            <img src={img} alt="Post Media" loading="lazy" className="w-full h-auto object-contain max-h-[500px]" onError={(e) => { e.target.parentElement.style.display = "none"; }} />
          </div>
        )}
        {video && (
          <div className="mb-3 rounded-lg sm:rounded-xl overflow-hidden bg-black">
            <video src={video} controls playsInline className="w-full max-h-[500px]" />
          </div>
        )}
      </div>
...
<div className="px-3 sm:px-4 md:px-5 pb-3 sm:pb-4 md:pb-5">
  <div className="flex items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 pt-2 sm:pt-3 border-t border-white/5 flex-wrap w-full">
    ...
    <button 
      ...
      style={{ boxShadow: "none", border: "none", background: "transparent", WebkitTapHighlightColor: "transparent" }}
    >
      <span className="material-symbols-outlined text-base sm:text-lg" style={{ fontVariationSettings: isLiked ? "'FILL' 1" : "'FILL' 0" }}>
        favorite
      </span>
    ...
    >
      ...
      <span className="text-[9px] sm:text-xs font-bold">
        {commentsList.length > 0 ? `(${commentsList.length})` : post.comment_count && !commentsList.length ? `(${post.comment_count})` : ""}
      </span>
    </button>
    ...
    <button 
      ...
      style={{ boxShadow: "none", border: "none", background: "transparent", WebkitTapHighlightColor: "transparent" }}
    >
      <span className="material-symbols-outlined text-base sm:text-lg" style={{ fontVariationSettings: isSaved ? "'FILL' 1" : "'FILL' 0" }}>
        bookmark
      </span>
      <span className="hidden sm:inline text-xs font-bold">
        Save
    ...
    <div className="mt-4 space-y-4">
            ...
            <div className="flex gap-2 items-start">
              <div className="flex-1 relative">
                ...
                <button onClick={handleAddComment} className="absolute right-2 sm:right-3 top-2 sm:top-2.5 text-emerald-600 dark:text-[#00ff88] hover:opacity-80 transition-opacity">
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
              {commentsList.length > 0 ? commentsList.map((comment) => (
                <div key={comment.id} className="flex gap-2 group">
                  <img alt={comment.author} className="w-7 h-7 rounded-full border border-slate-200 dark:border-white/10 object-cover shrink-0" src={comment.authorAvatar || avatar} onError={(e) => { e.target.src = avatar; }} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] sm:text-xs font-bold text-slate-900 dark:text-white pr-2 truncate">{comment.author}</span>
                      <div className="flex items-center gap-1 shrink-0">
                        <span className="text-[9px] text-slate-500 uppercase">{formatTimeAgo(comment.timestamp)}</span>
                        {String(comment.authorId) === String(currentUser.id) && (
                          <button onClick={() => handleDeleteComment(comment.id)} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 transition-all">
                            <span className="material-symbols-outlined text-xs">delete</span>
                          </button>
                        )}
                      </div>
                    </div>
                    <p className="text-[10px] sm:text-xs text-slate-700 dark:text-slate-300 mt-1 leading-relaxed">{comment.text}</p>
                  </div>
                </div>
              )) : (
                <p className="text-[10px] sm:text-xs text-slate-400 text-center py-3">No comments yet. Be the first!</p>
              )}
            </div>
          </div>
        )}
...
export const ResearchCard = memo(({ research }) => {
  ...
  const [isLiked, setIsLiked] = useState(() => {
    const saved = (() => { try { return JSON.parse(localStorage.getItem("postLikes") || "{}"); } catch { return {}; } })();
    return saved[resId] !== undefined ? saved[resId] : research.is_liked === "1";
  });
  const [likeCount, setLikeCount] = useState(parseInt(research.like_count || 0));
  const [isSaved, setIsSaved] = useState(() => {
    const saved = (() => { try { return JSON.parse(localStorage.getItem("savedPosts") || "{}"); } catch { return {}; } })();
    return saved[resId] !== undefined ? saved[resId] : research.is_saved === "1";
  });
  const [commentsOpen, setCommentsOpen] = useState(false);
  const [commentsList, setCommentsList] = useState([]);
  const [commentsLoaded, setCommentsLoaded] = useState(false);
  ...
  const handleToggleLike = async () => {
    const token = getLocalAuthToken();
    const wasLiked = isLiked;
    setIsLiked(!wasLiked); setLikeCount((c) => wasLiked ? Math.max(0, c - 1) : c + 1);
    const stored = (() => { try { return JSON.parse(localStorage.getItem("postLikes") || "{}"); } catch { return {}; } })();
    stored[resId] = !wasLiked; localStorage.setItem("postLikes", JSON.stringify(stored));
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/research/like-research/${resId}`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
      const result = await res.json();
      if (!result.status) {
  ...
  const handleToggleSave = async () => {
    ...
    setIsSaved(!wasSaved);
    const stored = (() => { try { return JSON.parse(localStorage.getItem("savedPosts") || "{}"); } catch { return {}; } })();
    stored[resId] = !wasSaved; localStorage.setItem("savedPosts", JSON.stringify(stored));
    if (wasSaved) window.dispatchEvent(new CustomEvent("postUnsaved", { detail: { postId: resId } }));
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/research/research-save/${resId}`, {
        method: "GET", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
      const result = await res.json();
      if (!result.status) {
        setIsSaved(wasSaved); stored[resId] = wasSaved; localStorage.setItem("savedPosts", JSON.stringify(stored));
      }
    } catch {}
  };
  ...
  const fetchComments = async () => {
    const token = getLocalAuthToken();
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/research/get-comments/${resId}`, { headers: { Authorization: `Bearer ${token}` } });
      const result = await res.json();
      const fetched = (result.data || []).map((c) => ({
        id: c.id, text: c.comment, author: c.name, authorId: c.user_id,
        authorAvatar: c.profile_image ? `${API_CONFIG.BASE_URL}/${c.profile_image}` : null, timestamp: c.created_at,
  ...
  const handleAddComment = async () => {
    ...
    const token = getLocalAuthToken();
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/research/add-comment/${resId}`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ comment: clean }),
      });
      const result = await res.json();
  ...
  const handleDeleteComment = async (commentId) => {
    setCommentsList((prev) => prev.filter((c) => c.id != commentId));
    const token = getLocalAuthToken();
    try { await fetch(`${API_CONFIG.BASE_URL}/research/delete-comment/${commentId}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); } catch {}
  };
  ...
  return (
    <article className="bg-white dark:bg-[#020f0a] rounded-lg sm:rounded-xl border border-white/5 shadow-sm overflow-visible relative w-full">
      <div className="p-3 sm:p-4 md:p-5">
        <div className="flex items-start gap-2 sm:gap-3 mb-3">
          <img alt={research.name || "User"} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-[#00ff88]/20 object-cover shrink-0" src={getPostProfileSrc(research)} onError={(e) => { e.target.src = avatar; }} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-white truncate capitalize">{research.name || "User"}</p>
              <span className="inline-flex items-center gap-1 rounded-full bg-[#00ff85]/10 text-[#00c46a] dark:text-[#00ff85] border border-[#00ff85]/20 px-2 py-0.5 text-[10px] font-bold shrink-0">
                <span className="material-symbols-outlined text-xs">science</span>Research
              </span>
            </div>
            <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 capitalize">{research.user_type === "institute" ? "Institute" : "Individual"} • {formatActivityDate(research.created_at)}</p>
          </div>
        </div>
        <h4 className="mb-2 text-xs sm:text-sm md:text-base font-bold text-slate-900 dark:text-white">{research.research_title}</h4>
  ...
  <div className="mb-3">
    ...
    {abs.length > 120 && ( // Ek andaze se character safety lagayi hai taaki chote text par button na dikhe
      ...
      >
        ...
        {keywords.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {keywords.map((k, i) => (<span key={i} className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/40 px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-medium">{k}</span>))}
          </div>
        )}
        {fileUrl && (
          <div className="mb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 rounded-lg sm:rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-[#0e0f10] p-2.5 sm:p-3">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-slate-200 dark:bg-[#0f172a] border border-emerald-200 dark:border-[#00ff88]/20 flex items-center justify-center shrink-0">
                <span className="material-symbols-outlined text-emerald-600 dark:text-[#00ff88] text-lg sm:text-xl">description</span>
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-white truncate">{research.research_file.split("/").pop() || "Research Paper"}</p>
                <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5">PDF Document</p>
              </div>
            </div>
            <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto px-3 sm:px-4 py-1.5 sm:py-2 bg-[#00ff88] text-black font-bold text-xs sm:text-sm rounded-lg hover:bg-[#00dd77] transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
              <span className="material-symbols-outlined text-xs sm:text-sm">open_in_new</span>View
            </a>
          </div>
        )}
      </div>
      ...
      <div className="px-3 sm:px-4 md:px-5 pb-3 sm:pb-4 md:pb-5">
        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 pt-2 sm:pt-3 border-t border-white/5 flex-wrap w-full">
          <button onClick={handleToggleLike} className={`flex items-center gap-0.5 sm:gap-1 transition-colors text-xs sm:text-sm ${isLiked ? "text-emerald-600 dark:text-[#00ff88]" : "text-slate-500 hover:text-emerald-600 dark:text-[#00ff88]"}`}>
            <span className="material-symbols-outlined text-base sm:text-lg" style={{ fontVariationSettings: isLiked ? "'FILL' 1" : "'FILL' 0" }}>favorite</span>
            <span className="text-[9px] sm:text-xs font-bold">{likeCount > 0 ? likeCount : "Like"}</span>
          </button>
          <button onClick={handleToggleComments} className={`flex items-center gap-0.5 sm:gap-1 transition-colors text-xs sm:text-sm ${commentsOpen ? "text-emerald-600 dark:text-[#00ff88]" : "text-slate-500 hover:text-emerald-600 dark:text-[#00ff88]"}`}>
            <span className="material-symbols-outlined text-base sm:text-lg">chat_bubble</span>
            <span className="text-[9px] sm:text-xs font-bold">{commentsList.length > 0 ? `(${commentsList.length})` : research.comment_count && !commentsList.length ? `(${research.comment_count})` : ""}</span>
          </button>
   
          <button onClick={handleToggleSave} className={`ml-auto flex items-center gap-0.5 sm:gap-1 transition-colors text-xs sm:text-sm ${isSaved ? "text-emerald-600 dark:text-[#00ff88]" : "text-slate-500 hover:text-emerald-600 dark:text-[#00ff88]"}`}>
            <span className="material-symbols-outlined text-base sm:text-lg" style={{ fontVariationSettings: isSaved ? "'FILL' 1" : "'FILL' 0" }}>bookmark</span>
  ...
  return (
    <article className="bg-white border border-gray-200 text-slate-800 dark:bg-[#141414] dark:border-white/10 dark:text-white rounded-2xl shadow-sm overflow-hidden">
      <div className="p-4 sm:p-5">
        ...
        <h4 className="mt-4 text-base sm:text-lg font-bold text-slate-900 dark:text-white">{localPoll.question}</h4>
...
const UserProfile = ({
  ...
  const initialConnectionStatus = propInitialStatus ?? routeState.initialConnectionStatus ?? 3;
  ...
  const [userType, setUserType] = useState(() => {
    ...
    const isInstitute =
      user?.type === "Research Institute" || user?.organization_type ||
      user?.institute_name || user?.user_type === "institute";
  ...
  const [openSections, setOpenSections] = useState({
    personal: true, professional: true, institute: true, admin: true, contact: true,
  });
  ...
  const [activity, setActivity] = useState({ posts: [], research: [], polls: [] });
  ...
  const getCurrentUserId = () => {
    try {
      ...
      const u = JSON.parse(userStr);
      if (Array.isArray(u) && u.length > 0) return String(u[0].id || u[0].user_id || "");
  ...
  useEffect(() => {
    ...
    const fetchConnectionStatus = async () => {
      try {
        ...
        const foundInMyList = myList.status && Array.isArray(myList.data) &&
          myList.data.some((item) => {
            const id = item.id || item.user_id || item.connected_user_id;
            return String(id) === String(user.id);
          });
        const foundInProfileList = profileList.status && Array.isArray(profileList.data) &&
      ...
      } catch (err) { console.error("Connection status fetch error:", err); }
    };
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      try {
        const token = getAuthToken();
        const res = await fetch(`${API_CONFIG.BASE_URL}/user/connected-users-list-user/${user.id}`, {
          method: "GET",
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        });
        const result = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const token = getAuthToken();
        const timeoutId = setTimeout(() => { if (requestRef.current) requestRef.current.abort(); }, 5000);
        const response = await fetch(`${API_CONFIG.BASE_URL}/profile/get-chat-user-profile/${user.id}`, {
          method: "GET",
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        ...
        if (result.status && result.data) {
          const apiUserType = result.data.user?.user_type || (result.data.institute ? "institute" : "individual");
          setUserType(apiUserType);
          setProfileData(result.data);
          profileCache.set(cacheKey, { data: result.data, userType: apiUserType, time: Date.now() });
        }
      } catch (error) { if (error.name !== "AbortError") {} }
    };
    fetchData();
    return () => { if (requestRef.current) requestRef.current.abort(); };
  }, [user?.id]);
  ...
  useEffect(() => {
    const handleUpdate = (e) => {
      if (String(e.detail.userId) === String(user?.id)) setConnectionStatus(e.detail.status);
    };
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      ...
      try {
        const token = getAuthToken();
        const res = await fetch(`${API_CONFIG.BASE_URL}/post/user-created-content/${user.id}`, {
          method: "GET",
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        });
        const result = await res.json();
        if (!aborted && result.status && result.data) {
          setActivity({
            ...
            research: Array.isArray(result.data.research) ? result.data.research : [],
            polls: Array.isArray(result.data.polls) ? result.data.polls : [],
          });
        }
      } catch { /* non-critical */ } finally { if (!aborted) setActivityLoading(false); }
    };
    fetchActivity();
    return () => { aborted = true; };
  }, [user?.id]);
  ...
  const fetchConnectedUsers = async () => {
    ...
    try {
      ...
      if (result.status && result.data) {
        setConnectedUsersList(result.data);
        setConnectedCount(result.data.length);
      } else { setConnectedUsersList([]); setConnectedCount(0); }
    } catch { setConnectedCount(0); } finally { setLoadingConnected(false); }
  };
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.status) { toast.success(result.message || "User blocked successfully"); onClose(); }
      else toast.error(result.message || "Failed to block user");
    } catch { toast.error("Error blocking user. Please try again."); }
      ...
      try {
        if (connectionStatus === 2) {
          ...
          if (result.status) {
            ...
            window.dispatchEvent(new CustomEvent("connectionStatusUpdated", { detail: { userId: String(user.id), status: 3 } }));
          }
        } else {
          ...
          if (result.status) {
            ...
            profileCache.delete(`profile_${user.id}`);
            window.dispatchEvent(new CustomEvent("connectionStatusUpdated", { detail: { userId: String(user.id), status: 1 } }));
          }
        }
  ...
  const buildExperience = () => {
    ...
    const len = Math.max(companies.length, roles.length, durations.length, descriptions.length);
    const list = [];
    for (let i = 0; i < len; i++) {
      const entry = {
        company: companies[i] || "",
      ...
      if (entry.company || entry.role || entry.duration || entry.description) list.push(entry);
    }
    return list;
  };
  ...
  const regId =
    ...
    (user?.registration_id && String(user?.registration_id) !== String(user?.id) ? user.registration_id : "") ||
    user?.registration_no ||
  ...
  const aboutText =
    userType === "institute"
      ? instituteData.institute_description || instituteProfileData.institute_description
      : individualData.short_bio;

  const locationSummary =
    userType === "institute"
      ? [instituteProfileData.city, instituteProfileData.state, instituteProfileData.country].filter(Boolean).join(", ")
      : [individualData.city, individualData.state, individualData.country].filter(Boolean).join(", ");
  ...
  const personalHasContent =
    hasVal(userData.name) || hasVal(individualData.date_of_birth) ||
    hasVal(individualData.short_bio) || hasVal(individualData.language) ||
    hasVal(individualData.country) || hasVal(individualData.state) ||
    hasVal(individualData.city) || hasVal(individualData.pincode);

  const professionalHasContent =
    hasVal(individualData.describes) || hasVal(individualData.current_research) ||
    hasVal(individualData.year_of_experience_vijay || individualData.year_of_experience) ||
    hasVal(individualData.skills_vijay || individualData.skills) ||
    hasVal(individualData.previous_publication_vijay || individualData.previous_publication) ||
  ...
  const instituteHasContent =
    hasVal(instituteData.institute_name) || hasVal(instituteProfileData.organization_type) ||
    hasVal(instituteProfileData.country) || hasVal(instituteProfileData.state) ||
    hasVal(instituteProfileData.city) || hasVal(instituteData.address) ||
  ...
  const contactHasContent =
    ...
    hasVal(userType === "institute" ? instituteData.contact_no : individualData.contact_no) ||
    hasVal(userType === "institute" ? instituteData.website : individualData.website) ||
    hasVal(userType === "institute" ? instituteProfileData.linkedin : individualData.linkedin) ||
    hasVal(userType === "institute" ? instituteProfileData.research_gate : individualData.research_gate) ||
    hasVal(userType === "institute" ? instituteProfileData.orc_id : individualData.orc_id) ||
    hasVal(userType === "institute" ? instituteProfileData.personal_website : individualData.personal_website);

  /* ── 2 most-recent items for preview (newest-first assumed from API) ── */
  const recentItems = [
    ...activity.posts.slice(0, 2).map((p) => ({ type: "post", data: p, time: p.created_at })),
    ...activity.research.slice(0, 2).map((r) => ({ type: "research", data: r, time: r.created_at })),
    ...activity.polls.slice(0, 2).map((p) => ({ type: "poll", data: p, time: p.created_at })),
  ]
    .sort((a, b) => new Date(b.time || 0) - new Date(a.time || 0))
    .slice(0, 2);

  const totalActivity = activity.posts.length + activity.research.length + activity.polls.length;
        ...
        <div className="relative overflow-hidden rounded-3xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#141414] shadow-sm mb-5">
          ...
          <div className="relative h-40 sm:h-52 w-full bg-gradient-to-br from-[#00ff85]/30 via-emerald-400/10 to-transparent dark:from-[#00ff85]/20 dark:via-emerald-500/5 dark:to-transparent">
            <div
              ...
              style={{ backgroundImage: "radial-gradient(circle, rgba(0,255,133,0.18) 1px, transparent 1px)", backgroundSize: "22px 22px" }}
            />
              ...
              <div className="relative -mt-20 sm:-mt-24 shrink-0 self-center lg:self-auto">
                <div className="rounded-full p-[3px] bg-gradient-to-br from-[#00ff85] to-emerald-500 shadow-xl">
                  <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-full ring-4 ring-white dark:ring-[#141414] bg-gray-100 dark:bg-[#13231a] overflow-hidden">
                    <img className="w-full h-full rounded-full object-cover" src={getProfileImage()} alt={getDisplayName()} loading="lazy" onError={(e) => { e.target.src = avatar; }} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0 text-center lg:text-left">
                ...
                <div className="mt-2.5 flex flex-wrap items-center justify-center lg:justify-start gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[#00ff85]/10 text-[#00c46a] dark:text-[#00ff85] border border-[#00ff85]/30 px-3 py-1 text-xs font-semibold">
                    <MaterialIcon name={userType === "individual" ? "verified" : "domain"} className="text-sm" />
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 px-3 py-1 text-xs font-medium">
                    ...
                    Registration ID:&nbsp;
                    <span className="font-bold text-slate-700 dark:text-slate-200">{regId}</span>
                  </span>
                </div>
              </div>
                ...
                {!isOwnProfile ? (
                  <>
                    ...
                    >
                      ...
                      <span>{connectionStatus === 2 ? "Connected" : connectionStatus === 1 ? "Pending" : "Connect"}</span>
                    </button>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (menuButtonRef.current) {
                            ...
                            setMenuButtonPos({ top: rect.bottom + 8, right: window.innerWidth - rect.right });
                          }
    ...
    <span>Edit Profile</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
          ...
          <aside className="space-y-5 lg:sticky lg:top-6">
            {aboutText && (
              <SideCard icon="info" title="About">
                <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300 whitespace-pre-line">{aboutText}</p>
              </SideCard>
            )}
            ...
            <SideCard icon="group" title="Connections">
                  ...
                  <div className="flex -space-x-2">
                    {connectionPreview.map((u) => {
                      const imgSrc = u.profile_image
                        ? u.profile_image.startsWith("http") ? u.profile_image : `${API_CONFIG.BASE_URL}/${u.profile_image}`
                        : null;
                      return (
                        <img key={u.user_id} src={imgSrc || avatar} alt={u.name}
              ...
              <button onClick={handleOpenConnectedPopup} className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 dark:border-white/10 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-white/5 hover:border-[#00ff85]/40 transition-colors">
            ...
            >
              ...
              <FieldInfo label="Phone Number" value={userType === "institute" ? instituteData.contact_no : individualData.contact_no} icon="phone" />
          ...
          <main className="lg:col-span-2 space-y-5">
            ...
            {userType === "individual" && (
              <SectionCard icon="person" title="Personal Information" isOpen={openSections.personal} onToggle={() => toggleSection("personal")} hasContent={personalHasContent}>
            ...
            {/* Professional Info */}
            {userType === "individual" && (
              <SectionCard icon="school" title="Professional Information" isOpen={openSections.professional} onToggle={() => toggleSection("professional")} hasContent={professionalHasContent}>
                ...
                {(parseArrayField(individualData.developement_goals).length > 0 || parseArrayField(individualData.interest).length > 0) && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <TagsField icon="eco" label="Sustainable Development Goals" items={parseArrayField(individualData.developement_goals)} />
                    <TagsField icon="diversity_3" label="Collaboration Interests" items={parseArrayField(individualData.interest)} />
                  </div>
                )}
                <HighlightField icon="trending_up" label="Current Research Level" value={individualData.current_research} />
            ...
            {/* Institute Details */}
            {userType === "institute" && (
              <SectionCard icon="domain" title="Institute Details" isOpen={openSections.institute} onToggle={() => toggleSection("institute")} hasContent={instituteHasContent}>
            ...
            {/* Admin Info */}
            {userType === "institute" && (
              <SectionCard icon="admin_panel_settings" title="Administrator Information" isOpen={openSections.admin} onToggle={() => toggleSection("admin")} hasContent={adminHasContent}>
            ...
            <div className="bg-white border border-gray-200 dark:bg-[#141414] dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 border-b border-gray-200 dark:border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-slate-900 dark:text-white font-bold text-base sm:text-lg">Activity</h3>
                ...
                <div className="flex items-center gap-2 flex-wrap sm:justify-end">
                  {stats.map((s) => (
                    ...
                    >
                      <MaterialIcon name={s.icon} className="text-sm text-[#00c46a] dark:text-[#00ff85]" />
                      <span className="font-extrabold text-slate-900 dark:text-white">{s.value}</span>
              ...
              <div className="p-4 sm:p-5 bg-gray-50/50 dark:bg-[#0a120e]/40">
                {activityLoading ? (
                  <div className="flex items-center justify-center gap-3 py-10">
                    ...
                    <span className="text-slate-500 dark:text-slate-400 text-sm">Loading activity...</span>
                  </div>
                ) : recentItems.length === 0 ? (
                  <ActivityEmpty icon="dynamic_feed" text="No activity to show yet." />
                ) : (
                  <div className="space-y-4">
                    {recentItems.map((item) => {
                      if (item.type === "post") return <PostCard key={`post-${item.data.id}`} post={item.data} onShareClick={(id) => { setSelectedSharePostId(id); setIsShareOpen(true); }} />;
                      if (item.type === "research") return <ResearchCard key={`res-${item.data.research_id}`} research={item.data} />;
                      return <PollCard key={`poll-${item.data.poll_id}`} poll={item.data} />;
                    })}
                  </div>
                )}
              </div>
              ...
              {totalActivity > 0 && (
                <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-1 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-[#141414]">
                  ...
                  >
                    ...
                    <MaterialIcon name="arrow_forward" className="text-base group-hover:translate-x-0.5 transition-transform" />
                  </button>
                </div>
              )}
            </div>
      ...
      <style jsx>{`
        ...
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(10px) scale(0.97); } to { opacity: 1; transform: translateY(0) scale(1); } }
        .animate-fadeInUp { animation: fadeInUp 0.2s ease-out; }
      `}</style>

      {/* ── Connected Users Popup (unchanged) ── */}
      {showConnectedPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowConnectedPopup(false)}>
          <div className="bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#00ff85]/20 rounded-2xl w-full max-w-sm shadow-2xl animate-fadeInUp overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-white/5">
              <div className="flex items-center gap-2">
                <MaterialIcon name="group" className="text-[#00c46a] dark:text-[#00ff85] text-xl" />
              ...
              <button onClick={() => setShowConnectedPopup(false)} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors p-1">
                ...
                <div className="divide-y divide-gray-200 dark:divide-white/5">
                  {connectedUsersList.map((u) => {
                    const imgSrc = u.profile_image
                      ...
                      : null;
                    return (
                      <div key={u.user_id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                        <div className="w-10 h-10 rounded-full border border-[#00ff85]/20 overflow-hidden shrink-0 bg-gray-200 dark:bg-[#13231a]">
                          <img src={imgSrc || avatar} alt={u.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = avatar; }} />
                        </div>
                        <div className="flex-1 min-w-0">
