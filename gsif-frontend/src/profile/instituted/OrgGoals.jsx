...
const OrgGoals = ({ progress, onNext, onBack }) => {
  ...
  const initialGoals = [
    {
      id: "share_publish",
      title: "Share & publish research work",
      desc: "Showcase your latest findings to the global sustainability community.",
    },
    {
      id: "visibility",
      title: "Increase visibility of our research",
      desc: "Expand the reach of your institutional or individual research impact.",
    },
    {
      id: "connect",
      title: "Connect with researchers or institutions",
      desc: "Build meaningful professional relationships within your field.",
    },
    {
      id: "collaborators",
      title: "Find collaborators",
      desc: "Discovery partners for upcoming interdisciplinary research projects.",
    },
    {
      id: "host_initiatives",
      title: "Host sustainability initiatives",
      desc: "Organize and manage environmental projects or campaigns.",
    },
  ];
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem(
      "orgStep4",
      JSON.stringify({ platform: selectedTitles }),
    );
    if (onNext) onNext();
  };
  ...
  return (
    <main
      className="
min-h-screen flex flex-col items-center py-12 md:py-16 px-4 md:px-0 font-sans
...
dark:text-white/60 dark:hover:text-white transition-colors text-xs font-semibold flex items-center gap-1 mt-0.5"
        ...
        <div className="text-center">
          <h1
            className="text-3xl md:text-[40px] font-bold mb-4 tracking-tight leading-tight
...
"
          ...
          <p className="text-lg text-gray-600 dark:text-white/40">
            This helps us personalize your dashboard and surface relevant
            opportunities.
          </p>
        </div>
...
${
  isSelected
    ? "border-[#00ff88] bg-green-100 text-slate-900 dark:bg-[#1a2e25] dark:text-white"
    : "border-gray-300 bg-gray-100 hover:border-[#00ff88]/40 dark:border-[#31684e] dark:bg-[#1a2e25]/50 dark:text-white"
}`}
                >
                  <div
                    className={`mt-1 flex-shrink-0 size-5 rounded-md border-2 flex items-center justify-center transition-all ${
                      isSelected
                        ...
                        : "border-gray-400 dark:border-[#31684e]"
                    }`}
                  >
                    {isSelected && (
                      <span className="material-symbols-outlined text-[14px] text-[#0b1410] font-bold">
                        check
                      </span>
                    )}
                  </div>
                  <div className="flex-grow pr-8">
                    <p className="font-bold text-[17px] text-slate-900 dark:text-white">
                      {goal.title}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-[#90cbaf] mt-0.5">
                      {goal.desc}
                    </p>
                  </div>
                  {isCustom && (
          ...
          <div className="flex flex-col gap-3">
            {!showOtherInput ? (
              <button
                ...
                className="
flex items-center justify-center gap-2 py-5 border-2 border-dashed rounded-xl transition-all group

border-gray-300 bg-gray-100
hover:border-[#00ff88] hover:bg-gray-200

dark:border-[#31684e] dark:bg-[#1a2e25]/30 dark:hover:bg-[#1a2e25]
"
              >
                <span className="material-symbols-outlined text-[#00ff88]">
                  add_circle
                </span>
                <span className="font-bold text-[#90cbaf] group-hover:text-[#00ff88]">
                  Add something else
                </span>
              </button>
            ) : (
              <div className="flex gap-2">
                <input
                  ...
                  autoFocus
                  className="
flex-grow border-2 rounded-xl py-3.5 px-4

bg-gray-100 border-gray-300 text-slate-900
dark:bg-[#1a2e25] dark:border-[#00ff88] dark:text-white

outline-none focus:outline-none focus:ring-2 focus:ring-[#00ff88]/30 focus:border-[#00ff88]
"
                  placeholder="Describe your goal..."
                  value={otherText}
                  onChange={(e) => setOtherText(e.target.value)}
                  onKeyDown={(e) =>
                    e.key === "Enter" && (e.preventDefault(), addCustomGoal())
                  }
                />
                <button
                  type="button"
