...
const InstituteProfile = () => {
  ...
  try {
    const token = getAuthToken();
    if (!token) return;
    const response = await fetch(`${API_CONFIG.BASE_URL}/user/connected-users-list`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
...
<div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto mt-4 md:mt-0">              <button
    ...
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[80vh]">
      <div className="flex items-center justify-between p-4 sm:p-6 border-b border-[#1a1a1a]">
        <h3 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          Connections : <span className="text-white font-bold">{connectionCount}</span>
        </h3>
        <button onClick={() => setShowConnectionsModal(false)} className="text-slate-400 hover:text-white transition-colors">
      ...
      <div className="overflow-y-auto p-2 sm:p-4 space-y-2 flex-1">
        {connectedUsersList.length > 0 ? (
          connectedUsersList.map((connUser) => (
            <div key={connUser.id} className="flex items-center gap-4 p-3 bg-[#111] border border-[#1a1a1a] rounded-xl hover:border-[#0df287]/50 transition-colors">
              <img
