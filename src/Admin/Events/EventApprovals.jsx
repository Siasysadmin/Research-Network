...
const ActionMenu = ({ status, onViewDetails, onEdit, onDuplicate, onViewReason, onDelete, onClose }) => {
  ...
  return (
    <div className="absolute right-0 top-8 sm:top-10 w-40 sm:w-44 bg-[#13231a] border border-[#1e3a2c] rounded-lg sm:rounded-xl shadow-xl z-50 py-1 overflow-hidden text-xs sm:text-sm">
      <button onClick={onViewDetails} className="w-full flex items-center gap-2.5 px-3 sm:px-4 py-2 sm:py-2.5 text-slate-300 hover:bg-white/5 transition-colors">
        <MaterialIcon name="visibility" className="text-sm sm:text-base text-slate-400 shrink-0" />
    ...
    <Layout activeNav={activeNav} setActiveNav={setActiveNav}>
      <div className="w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8 overflow-x-hidden">
        <div className="mb-6 sm:mb-8 lg:mb-10 mt-2 sm:mt-4">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1 sm:mb-2">Event Approvals</h1>
          <p className="text-xs sm:text-sm md:text-base text-slate-400">Review and approve user submitted events</p>
        </div>

        {loading && <div className="flex justify-center py-20 text-slate-400">Loading events...</div>}
              ...
              return (
                <div key={event.id} className="bg-[#0d1a12] border border-white/5 hover:border-white/10 transition-colors rounded-lg sm:rounded-2xl p-3 sm:p-4 md:p-6 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 md:gap-5 relative overflow-hidden">
                  ...
                  <div className="flex-1 min-w-0 order-1 sm:order-2">
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-white line-clamp-2">{event.title}</h3>
                    <p className="text-[10px] sm:text-xs text-slate-500 mb-2 sm:mb-1.5">by {event.submittedBy}</p>
                    <div className="flex flex-col gap-1 sm:gap-0.5 text-xs sm:text-sm text-slate-400">
                      <div className="flex items-center gap-1.5 min-w-0 truncate">
                        <MaterialIcon name="schedule" className="!text-sm sm:!text-base shrink-0" />
                        <span className="truncate text-[10px] sm:text-xs">{event.time}</span>
                      </div>
                      <div className="flex items-center gap-1.5 min-w-0 truncate">
                        <MaterialIcon name="location_on" className="!text-sm sm:!text-base shrink-0" />
                        <span className="truncate text-[10px] sm:text-xs">{event.location}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    {isPending && (
                      <div className="flex items-center gap-2 mt-1 w-full sm:w-auto">
                        <button 
                          ...
                          className="flex-1 sm:flex-none px-2.5 sm:px-4 py-1.5 sm:py-1.5 rounded-lg border border-[#32ff99]/40 text-[#32ff99] hover:bg-[#32ff99]/10 transition-colors text-[9px] sm:text-xs font-semibold whitespace-nowrap"
                        >
                  ...
                  <div className="absolute top-3 right-3 sm:relative sm:top-auto sm:right-auto shrink-0 order-4 sm:order-4" ref={openMenuId === event.id ? menuRef : null}>
                    <button onClick={() => setOpenMenuId(openMenuId === event.id ? null : event.id)} className="p-1 text-slate-500 hover:text-white transition-colors rounded-lg hover:bg-white/5">
      ...
      {showApproveModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-[#0d1f16] border border-[#32ff99]/30 rounded-2xl p-6 sm:p-8 w-full max-w-[400px] shadow-2xl text-center">
            <div className="w-16 h-16 rounded-full bg-[#32ff99]/10 border border-[#32ff99]/30 flex items-center justify-center mx-auto mb-4">
              <MaterialIcon name="check_circle" className="text-[#32ff99] text-3xl" />
            </div>
            <h2 className="text-xl font-bold text-white mb-2">Approve Event?</h2>
            <p className="text-slate-400 text-sm leading-relaxed mb-5">
              Are you sure you want to approve this event? It will be moved to approved status.
            </p>
            <div className="flex gap-3">
              <button onClick={closeModals} className="flex-1 py-2.5 rounded-xl border border-slate-500/30 text-slate-300 font-bold text-sm hover:bg-white/5 transition-all">
                Cancel
              </button>
              <button onClick={confirmApprove} className="flex-1 py-2.5 rounded-xl bg-[#32ff99]/10 border border-[#32ff99]/30 text-[#32ff99] font-bold text-sm hover:bg-[#32ff99]/20 transition-all">
                Yes, Approve
              </button>
            </div>
          </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-[#0d1f16] border border-red-500/30 rounded-2xl p-6 sm:p-8 w-full max-w-[400px] shadow-2xl text-center">
