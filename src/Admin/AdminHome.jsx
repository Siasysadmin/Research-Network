...
const MainContent = () => {
    ...
    const fetchFeed = async () => {
      try {
        ...
        const filteredApiPosts = apiPosts.filter((p) => {
          ...
          if (p.post_text && pollQuestions.has(p.post_text.trim())) return false;
          return true;
        });

        // Merge all feed items and sort by created_at descending
        const allItems = [...publishedResearch, ...polls, ...mockPosts, ...filteredApiPosts];
        allItems.sort((a, b) => {
  ...
  const handlePollVote = async (pollId, optionId) => {
    ...
    setFeedData((prev) =>
      prev.map((item) => {
        ...
        const updatedOptions = item.options.map((opt) => {
          ...
          if (prevVotedId && String(opt.id) === String(prevVotedId)) count = Math.max(0, count - 1);
          return { ...opt, vote_count: count, is_voted_by_me: String(opt.id) === String(optionId) ? 1 : 0 };
        });
        const totalVotes = updatedOptions.reduce((s, o) => s + parseInt(o.vote_count || 0), 0);
        return {
          ...item,
          ...
          options: updatedOptions.map((opt) => ({
            ...opt,
            percentage: totalVotes > 0 ? Math.round((parseInt(opt.vote_count || 0) / totalVotes) * 100) : 0,
          })),
        };
      })
    );
    ...
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/poll/vote-poll/${numericPollId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handlePollUndoVote = async (pollId) => {
    ...
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/poll/undo-vote/${numericPollId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const PollCard = ({ post }) => {
    ...
    const hasVoted = post.my_vote_option_id !== null && post.my_vote_option_id !== undefined;
    ...
    const postName =
      post.user_type === "institute"
        ...
        : post.name || "User";
    const postType = post.user_type === "institute" ? "Institute" : "Individual";
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="sm:ml-16 space-y-2 sm:space-y-3 mb-4">
            {(post.options || []).map((option) => {
              const isMyVote = String(post.my_vote_option_id) === String(option.id);
